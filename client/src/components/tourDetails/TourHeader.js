...
const TourHeader = ({ tour }) => {
    return (
        ...
        <div className="heading-box">
            <h1 className="heading-primary"><span>
                    {tour && tour.name.split(' ').splice(0, 2).join(' ')}<br />{tour && tour.name.split(' ').splice(2).join('')}
                </span></h1>
            <div className="heading-box__group">
            <div className="heading-box__detail">
                <ImClock className="heading-box__icon"/>
                <span className="heading-box__text">{tour && tour.duration} days</span>
            </div>
            <div className="heading-box__detail">
